...
function beta = ol2(XXmat, cvec, inival, lambda, varset, maxiter, tol)
...
while (iter <= maxiter) & (update > tol)
    ...
    for k = 1:length(ind)
        ...
        % solve min_beta  (2 Lam)^(-1) (z - beta)^2 + p_lambda(beta^2)
        % for scalar beta
        n=size(XXmat,2)
        z1 = XXmat(I, I)+(lambda/n);
        Lam = 1/z1;
        if isempty(setr)
            z = cvec(I)/z1;
        else
