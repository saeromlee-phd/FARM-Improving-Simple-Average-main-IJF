function [fcsts, sel]=forecasts_FARM1_withc(y, data, org_y, org_data, mean, seed) % forecasts without considering factors
...
function [yf_c, sellasso, yfa_c, sellassoa]=Lassof_fcst_post(y,X, org_y, org_data, seed)
 rng(seed,'twister');   
    ...
    sumbeta=sum(betapost);
    yf_x(isnan(yf_x)) = 0;
    yf_c=(1-sumbeta)*org_y(end,:)+yf_x;
    else
    yf_c=org_y(end,:);
    end
    ...
    if sum(ind)>0
       ...
       yfa_c=(1-sumbetaposta)*org_y(end,:)+yfa_x;
       sellassoa=indaf;
 % report results
    else
        yfa_c=yf_c;
...
rng(seed,'twister');
    ...
    yf_c=(1-sumbeta_x)*org_y(end,:)+yf_0;
